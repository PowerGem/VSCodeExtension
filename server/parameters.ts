...
export const TARA_PARAMETERS: Parameter[] = [
...
export function getParametersByOptionBlock(blockName: string): Parameter[] {
  const normalizedBlock = blockName.toLowerCase();
  return TARA_PARAMETERS.filter(parameter => parameter.optionBlock.toLowerCase() === normalizedBlock);
}

export function findParameter(name: string, optionBlock?: string): Parameter | undefined {
  const normalizedName = name.toLowerCase();

  if (optionBlock) {
    const normalizedBlock = optionBlock.toLowerCase();
    const blockMatch = TARA_PARAMETERS.find(parameter =>
      parameter.name.toLowerCase() === normalizedName &&
  ...
  return TARA_PARAMETERS.find(parameter => parameter.name.toLowerCase() === normalizedName);
}

export function getValidOptionBlocks(): string[] {
  const blocks = new Set(TARA_PARAMETERS.map(p => p.optionBlock));
